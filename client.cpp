...
int main(int argc, char *argv[]) {
    ...
    cout << "Client starting with server IP: " << server_ip << endl;
    ...
    std::string pipeline_desc =
        ...
        "udpsink host=" + std::string(server_ip) + " port=5000 sync=false async=false "
        ...
        "udpsink host=" + std::string(server_ip) + " port=5001 sync=false async=false "
        "udpsrc port=5005 buffer-size=212992 ! srtpdec name=videortcprecvdec ! rtpbin_send.recv_rtcp_sink_0 "
        ...
        "udpsink host=" + std::string(server_ip) + " port=5002 sync=false async=false "
        ...
        "udpsink host=" + std::string(server_ip) + " port=5003 sync=false async=false "
        "udpsrc port=5007 buffer-size=212992 ! srtpdec name=audiortcprecvdec ! rtpbin_send.recv_rtcp_sink_1 "
        
        // --- RECEIVE PATH (from server) - OPTIMIZED ---
        "udpsrc port=5010 buffer-size=212992 ! "
        ...
        "udpsrc port=5011 buffer-size=212992 ! srtpdec name=videortcpdec ! rtpbin_recv.recv_rtcp_sink_0 "
        
        "udpsrc port=5012 buffer-size=212992 ! "
        ...
        "udpsrc port=5013 buffer-size=212992 ! srtpdec name=audiortcpdec ! rtpbin_recv.recv_rtcp_sink_1";
    ...
    cout << "\n=== LOW LATENCY Client with SRTP AES-256 running ===" << endl;
